...
export function computeAgeWithMonths(dateOfBirth: string): { years: number; months: number } {
  const dob = new Date(dateOfBirth)
  const today = new Date()
...
export function formatAge(dateOfBirth: string): string {
  const { years, months } = computeAgeWithMonths(dateOfBirth)
  const yearLabel = years === 1 ? "yr" : "yrs"
  const monthLabel = months === 1 ? "mon" : "mons"
  if (years === 0) return `${months} ${monthLabel} old`
  if (months === 0) return `${years} ${yearLabel} old`
  return `${years} ${yearLabel} & ${months} ${monthLabel} old`
}
